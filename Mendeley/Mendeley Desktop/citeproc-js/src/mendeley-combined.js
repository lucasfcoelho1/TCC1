...
CSL.Output.Formats["rtf"]["@display/left-margin"] = function (state, str) {
	// display-left-margin is formatted by Mendeley Desktop
	return "\\display-left-margin" + str + "\\tab ";
};

var Mendeley_citeprocReact;
...
if (isWebEngine()) {
	// For !isWebEngine() this happens on the processCitations function
	connectToMendeleyDesktop();
}

// Wraps functions exposed by Mendeley Desktop, providing explicit option names
// for better readability
var MendeleyDesktopWrapper = {
	setCitation: function (options) {
		MendeleyDesktop.setCitation(
			options.position,
			options.citation,
			options.citationId
		);
	},
	setBibliography: function (options) {
		MendeleyDesktop.setBibliography(
			options.bibliography,
			options.hangingIndent,
			options.hasBibliography
		);
	}
};
...
var localeAwareCaseInsensitiveStringCompare = function (s1, s2) {
	/* On WebKit comparing umlauts, accents, etc. using
	 * String.localeCompare doesn't work. On WebEngine it works correctly.
	 * This function uses WebEngine String.localeCompare() on WebEngine
	 * or the C++ implementation on WebKit. It's not possible to use the
	 * C++ implementation on WebEngine because it's async and citeproc-js
	 * expects the answer asyncrhonously.
	 */
	if (isWebEngine()) {
		return s1.toLowerCase().localeCompare(s2.toLowerCase());
	}
	else {
		return MendeleyDesktop.localeAwareCaseInsensitiveStringCompare(s1, s2);
	}
};

var getValueFromMap = function(map, key, defaultValue) {
	if (key in map) {
		return map[key];
	}
	else {
...
// and a bibliography
var processCitations = function (style, locale, citationClusters, items, enumerateCitations, reset, bibliographyOutputFormat) {
	if (!isWebEngine()) {
		// For isWebEngine() this happens on the load of this file
		MendeleyDesktop = MendeleyDesktop_;
	}
	try {
		if (!Mendeley_citeprocReact) {
			Mendeley_citeprocReact = new CiteprocReact({
				citeprocSys: {
					stringCompare: localeAwareCaseInsensitiveStringCompare,
					getAbbreviation: function (dummy, obj, jurisdiction, vartype, key) {
						if (vartype == "container-title") {
							var abbreviated = getValueFromMap(journalAbbreviations, key, key);
							obj["default"][vartype][key] = abbreviated;
						}
					},
					retrieveLocale: function(lang) {
						return getValueFromMap(locales, lang, getValueFromMap(locales, 'en-US', ''));
					}
				}
			});
		}
		...
		var renderOptions = {
			...
			items: items,
			enumerateCitations: enumerateCitations,
			reset: reset,
			bibliographyOutputFormat: bibliographyOutputFormat 
		};
		var result = Mendeley_citeprocReact.render(renderOptions);

		if (citationClusters.length === 0 && !result.formattedBibliography) {
			// Can't use "bibliographyOnly" mode for styles which don't have a bibliography element,
			// so as a workaround we need to add an in-line citation and use this as the
			// bibliography
			renderOptions.citationClusters = [{
					citationItems: [{
						id: _.keys(items)[0]
					}]
				}];
			result = Mendeley_citeprocReact.render(renderOptions);
		}

		// Pass result back to Mendeley Desktop
		_.each(result.formattedCitations, function (formattedCitation, index) {
			MendeleyDesktopWrapper.setCitation({
				position: index,
				citation: formattedCitation,
				citationId: "citationID not relevant"
			});
		});

		if (result.formattedBibliography) {
			MendeleyDesktopWrapper.setBibliography({
				bibliography: result.formattedBibliography,
				hangingIndent: (result.formattedBibliographyOptions && result.formattedBibliographyOptions.hangingindent)
					? true : false,
				hasBibliography: true
			});
		} else if (citationClusters.length === 0) {
			// This is a hack to allow Mendeley Desktop's "View library as citations" mode
			// to work with styles which don't include a bibliography
			MendeleyDesktopWrapper.setBibliography({
				bibliography: [result.formattedCitations[0]],
				hangingIndent: false,
				hasBibliography: true
			});
		} else {
			MendeleyDesktopWrapper.setBibliography({
                bibliography: ["This citation style does not include a bibliography."],
				hangingIndent: false,
