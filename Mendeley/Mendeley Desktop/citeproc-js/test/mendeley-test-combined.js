...
var _lineModeDiff = function (text1, text2) {
...
// Warning - input strings not permitted to contain newlines!
var diffStringLists = function (list1, list2) {
	var diffs = _lineModeDiff(list1.join('\n') + '\n', list2.join('\n') + '\n');
...
// This represents a list of citation clusters and can generate a list of citeproc-js
// commands to transform one list into another.
//
// In order to compare two ClusterLists, we perfom a line-based diff on them, where each
// line is a citation cluster JSON string.
//
// Citation clusters contain a list of reference item ID. For the diff to be useful, equivalent
// reference item IDs must represent the exact same reference metadata.
// This means that if the same reference is edited between renders, its ID must change, and
// it's treated as a completely new reference.
//
// To be sure of this, we don't trust any IDs provided by the client and generate each item
// ID by taking an MD5 hash of its metadata.
//
// This has the side effect of treating two references which may be distinct in a user's
// reference manager as the same reference item if they have the exact same metadata.

var ClusterList = function (options) {
	this.options = _.pick(options, [
			"citationClusters", // list   - citation clusters
			"items"	            // object - map of given itemIDs to items
		]);

	this.itemsByID = {};             // map of all new itemIDs (based on MD5 hashes) to items 
	this.nextCiteprocCitationID = 0; // counter which increments as each new citeproc-ready
	                                 // citation cluster is generated

	this.clusters = [];  // list of normalised citation clusters
	this.hashCache = {}; // cache of MD5 hash results

	this._prepareCitationClusters();
};

// Normalises all citation clusters, ready both for diffing and sending to citeproc
ClusterList.prototype._prepareCitationClusters = function () {
	var that = this;

	_.each(deepCopy(this.options.citationClusters), function (cluster) {
		_.each(cluster.citationItems, function (item) {
			if (!item.item) {
				item.item = that.options.items[item.id];
			}
			var preparedItem = that._prepareItem(item.item);
			item.id = preparedItem.id;
			delete item.item;
		});
		delete cluster.citationID;
		that.clusters.push({
			clusterString: JSON.stringify(_.pick(cluster, ['citationItems', 'properties'])),
			cluster: cluster
		});
	});
};

// Caching MD5 hashes offers speedup for documents with repeated reference items
ClusterList.prototype.MD5 = function (input) {
	var val = this.hashCache[input];
	if (val) {
		return val;
	}
	val = CryptoJS.MD5(input).toString();
	this.hashCache[input] = val;
	return val;
};

// Prepares an item. Gives it an ID which is a hash (MD5) of it's own contents
// and adds it to itemsByID
ClusterList.prototype._prepareItem = function (item) {
	var originalID = item.id;
	item.id = this.MD5(JSON.stringify(_.omit(item, 'id')));
	this.itemsByID[item.id] = item;
	return item;
};

// Gets an item based on the new hash-based ID, not IDs given by the client
ClusterList.prototype.getItem = function (itemID) {
	return this.itemsByID[itemID];
};

ClusterList.prototype._diffClusterLists = function (clusters1, clusters2) {
	return diffStringLists(
		_.pluck(clusters1, 'clusterString'),
		_.pluck(clusters2, 'clusterString'));
};

ClusterList.prototype.getCiteprocClusters = function () {
	var that = this;
	var result = _.pluck(this.clusters, 'cluster');
	
	// populate with citationIDs if they don't already exist
	_.each(result, function (cluster) {
		if (!cluster.citationID) {
			cluster.citationID = that._newCiteprocID();
		}
...
// requires citeprocClusters() to have been called already
ClusterList.prototype.getOrderedItemIDs = function () {
	var orderedItemIDs = [],
		insertedItems = {};

	_.each(this.clusters, function (cluster) {
		_.each(cluster.cluster.citationItems, function (item) {
			if (!insertedItems[item.id]) {
...
ClusterList.prototype._newCiteprocID = function () {
	var citationID = "CITATION-" + this.nextCiteprocCitationID;
	this.nextCiteprocCitationID++;
	return citationID;
};

ClusterList.prototype.getCluster = function (citationID) {
	return _.find(this.clusters, function (cluster) {
		return cluster.cluster.citationID === citationID;
	});
};

ClusterList.prototype.minimalCiteprocChanges = function (previousClusterList) {
	var that = this;

	// start indexing where the previous list left off
	this.nextCiteprocCitationID = previousClusterList.nextCiteprocCitationID;

	var diffs = this._diffClusterLists(previousClusterList.clusters, this.clusters);
	...
	var addCiteprocChange = function (newCluster) {
		// next items
		var citationsPost = [];

		if (originalIndex + 1 < previousClusterList.clusters.length) {
			citationsPost = _.map(_.range(originalIndex + 1, previousClusterList.clusters.length), function (index) {
				return [previousClusterList.clusters[index].cluster.citationID, 0 /* footnote index */];
			});
		}

		if (!newCluster) {
			// citeproc NEEDS us to specify a new cluster, so grab an existing one from either pre or post
			if (citationsPre.length > 0) {
				newCluster = that.getCluster(citationsPre.pop()[0]).cluster;
			} else if (citationsPost.length > 0) {
				newCluster = previousClusterList.getCluster(citationsPost.splice(0, 1)).cluster;
			} else {
				console.error('Trying to delete all citations - not supported!');
			}
		}

		newCluster.properties = newCluster.properties || {};

		citeprocChanges.push({
			citation: newCluster,
			citationsPre: _.clone(citationsPre),
			citationsPost: citationsPost
		});
		citationsPre.push([newCluster.citationID, 0 /* footnote index */]);
	...
	_.each(diffs, function (diff) {
		...
		switch (diff.action) {
			case "equal":
				_.each(diff.items, function (clusterString) {
					currentIndex++;
					originalIndex++;
					var citationID = previousClusterList.clusters[originalIndex].cluster.citationID;
					that.clusters[currentIndex].cluster.citationID = citationID;
					citationsPre.push([citationID, 0 /* footnote index */]);
				});
				break;
			case "remove":
				// don't add to new list
				originalIndex += diff.items.length;
				itemsDeleted = true;
				break;
			case "insert":
				_.each(diff.items, function (clusterString) {
					...
					var newCluster = that.clusters[currentIndex].cluster;
					newCluster.citationID = that._newCiteprocID();
					addCiteprocChange(newCluster);

					itemsDeleted = false;
				});
				break;
			default:
				throw new Error("Unexpected diff action: " + diff.action);
		}
	});
	...
	return citeprocChanges;
};

"use strict";

// This wrapper for citeproc provides the high performance of incremental updates, without
// the added complexity.
//
// 1. It uses processCitationCluster() calls to insert and remove citation clusters
//    efficiently instead of re-rendering the entire document.
// 2. The API is conceptually simple, allowing clients to send the entire list of citation
//    clusters on each render, without worrying about maintaining state.
//
// Incremental rendering works by performing a diff between the new and old document states,
// and generating the minimal list of processCitationCluster() calls required to transform
// the previous list of clusters to the list.
//
// A full re-render can be triggered by either changing the citation style, or specifiying
// the {reset: true} option.
//
// If reference items are specified, but no citationClusters are specified, a bibliography
// only render mode is triggered which is slightly faster. (Note: in future it would be
// nice to add the ability to specify a mix of cited and uncited references, which is
// supported by citeproc-js.)
//
// Documentation for citeproc-js: http://gsl-nagoya-u.net/http/pub/citeproc-doc.html

var CiteprocReact = function (options) {
	var that = this;

	options = _.pick(options, [
			'citeprocSys' // object - configuration for citeproc
		]) || {};

	this.clusterList;         // ClusterList - for the current render
	this.previousClusterList; // ClusterList - for the previous render
	this.style;               // string      - the CSL style
	this.locale;              // string      - the locale ("en-US", "en-GB", etc.)
	this.citeproc;            // CSL.Engine  - citeproc-js instance

	this.formattedCitations;           // list   - inline citations from citeproc in order of
	                                   //          appearance in document
	this.formattedBibliography;        // list   - ordered bibliography entries from citeproc
	this.formattedBibliographyOptions; // object - options from citeproc
	this.renderInfo;                   // object - information about the previous render

	this.citeprocSys = options.citeprocSys;	// configuration for citeproc

	this.citeprocSys.retrieveItem = function (itemID) {
		var result = that.clusterList.getItem(itemID);
		if (!result && that.previousClusterList) {
			result = that.previousClusterList.getItem(itemID);
		}
		return result;
	};
};

CiteprocReact.prototype.render = function (renderOptions) {
	renderOptions = _.pick(renderOptions, [
			'style',             // string  - the CSL style (required)
			'locale',	     // string - e.g. "en-US", "en-GB", ...
			'citationClusters',  // list    - the list of citationClusters (see unitTests.js for examples)
			'items',             // object  - map of itemIDs to items (see unitTests.js for examples)
			                     //           to render a bibliography only, without in-line citations,
			                     //           use ordered IDs in the following format:
			                     //           "ITEM-1", "ITEM-2", ..., "ITEM-42", etc...
			'reset',             // boolean - if true, a full re-render is performed
			'enumerateCitations',// boolean - e.g. for IEEE: true (default) -> "[1]"
			                     //                          false          -> "citation-number"
			'bibliographyOutputFormat'// string - "rtf" or "html"
		]);

	var startTime = new Date().getTime();
	this._init(renderOptions);

	this.previousClusterList = this.clusterList;

	if (!renderOptions.citationClusters || renderOptions.citationClusters.length === 0) {
		this._bibliographyOnlyCiteprocRender(renderOptions);
	} else {
		this.clusterList = new ClusterList({
			citationClusters: renderOptions.citationClusters,
			items: renderOptions.items
		});
		if (renderOptions.reset || !this.previousClusterList) {
			this._fullCiteprocRender(renderOptions);
		} else {
			this._incrementalCiteprocRender(renderOptions);
		}
	}

	this.renderInfo.timeTaken = new Date().getTime() - startTime;

	var result = {
		formattedCitations: _.pluck(this.formattedCitations, 'formattedCitation'),
		formattedBibliography: this.formattedBibliography,
		formattedBibliographyOptions: this.formattedBibliographyOptions,
...
CiteprocReact.prototype._init = function (renderOptions) {
	if (renderOptions.reset || !this.citeproc || this.style != renderOptions.style ||
			this.locale != renderOptions.locale || renderOptions.citationClusters.length === 0) {
		delete this.clusterList;
		delete this.previousClusterList;
		
		if (this.citeproc && this.style === renderOptions.style && this.locale === renderOptions.locale) {
			this.citeproc.restoreProcessorState([]);
		} else {
			this.style = renderOptions.style;
			this.locale = renderOptions.locale;
			this.citeproc = new CSL.Engine(this.citeprocSys,
					this.style,
					this.locale,
					true /* uses the specified locale even if the style has a default locale */
			);
			this.citeproc.setAbbreviations("default");
		}

		// Clear the abbreviation cache
		this.citeproc.transform.abbrevs = new CSL.AbbreviationSegments();
	}
};

// unlike the "full" and "incremental" renders, this expects the reference items
// to have ids in the form "ITEM-1", "ITEM-2", etc...
CiteprocReact.prototype._bibliographyOnlyCiteprocRender = function (renderOptions) {
	this.clusterList = {
		getItem: function (itemID) {
			return renderOptions.items[itemID];
		}
	};

	var orderedItemIDs = [];

	var itemRegExp = new RegExp("^ITEM-([0-9]+)$");

	// order items
	_.each(renderOptions.items, function (item) {
		var match = itemRegExp.exec(item.id);
		if (!match) {
			throw new Error('Items must have ID in the form "ITEM-XX" for bibliography only render');
		}
		orderedItemIDs[parseInt(match[1]) - 1] = item.id;
	});

	this.citeproc.updateItems([]);
	this.citeproc.updateUncitedItems(orderedItemIDs);

	this._renderBibliography(renderOptions);
	this.formattedCitations = [];

	// cleanup
	this.citeproc.updateUncitedItems([]);
	delete this.clusterList;
	delete this.previousClusterList;

	this.renderInfo = {
		type: 'bibliographyOnly'
	};
};

CiteprocReact.prototype._fullCiteprocRender = function (renderOptions) {
	var that = this;
	
	this.citeproc.setOutputFormat("html");

	this.citeproc.updateItems(this.clusterList.getOrderedItemIDs());
	var citeprocFormattedCitations = this.citeproc.rebuildProcessorState(this.clusterList.getCiteprocClusters());

	if (this.clusterList.clusters.length !== citeprocFormattedCitations.length) {
		throw new Error("formatted citations don't match input citations");
	}
	this.formattedCitations = _.map(citeprocFormattedCitations, function (citeprocResultCluster, index) {
		return {
			formattedCitation: citeprocResultCluster[2],
			citationID: that.clusterList.clusters[index].cluster.citationID
		};
	});
	this._renderBibliography(renderOptions);
	this.renderInfo = {
...
CiteprocReact.prototype._incrementalCiteprocRender = function (renderOptions) {
	var that = this;
	
	this.citeproc.setOutputFormat("html");

	var minimalChanges = this.clusterList.minimalCiteprocChanges(this.previousClusterList);
	_.each(minimalChanges, function (change) {
		var formattedCitationChanges =
			that.citeproc.processCitationCluster(change.citation, change.citationsPre, change.citationsPost)[1];

		var preCitationIDs = _.pluck(change.citationsPre, 0);
		var postCitationIDs = _.pluck(change.citationsPost, 0);

		var newFormattedCitations = _.filter(that.formattedCitations, function (formattedCitation) {
			return _.contains(preCitationIDs, formattedCitation.citationID);
		});
		newFormattedCitations.push({citationID: change.citation.citationID});
		newFormattedCitations = newFormattedCitations.concat(_.filter(that.formattedCitations, function (formattedCitation) {
			return _.contains(postCitationIDs, formattedCitation.citationID);
		}));
		that.formattedCitations = newFormattedCitations;

		_.each(formattedCitationChanges, function (formattedCitationChange) {
			that.formattedCitations[formattedCitationChange[0]].formattedCitation = formattedCitationChange[1];
		});
	});
	...
	this.renderInfo = {
		type: 'incremental',
		numChanges: minimalChanges.length
	};
};

CiteprocReact.prototype._renderBibliography = function (renderOptions) {
	var citationNumberString;
	
	this.citeproc.setOutputFormat(renderOptions.bibliographyOutputFormat);

	if (!renderOptions.enumerateCitations) {
		citationNumberString = "citation-number";
	}
	
	var citeprocFormattedBibliography = this.citeproc.makeBibliography(citationNumberString);
	this.formattedBibliographyOptions = citeprocFormattedBibliography[0];
	this.formattedBibliography = citeprocFormattedBibliography[1];
};
...
// Connects to Mendeley Desktop
var MendeleyDesktop = null;

var isWebEngine = function () {
    return navigator.appVersion.indexOf("QtWebEngine") != -1;
};

var connectToMendeleyDesktop = function () {
    if (MendeleyDesktop == null) {
        new QWebChannel(qt.webChannelTransport, function (channel) {
            MendeleyDesktop = channel.objects.MendeleyDesktop;
            MendeleyDesktop.setEngineReady(true);
        });
    }
}

// End of connecting to Mendeley Desktop

// UnitTest
// Example use: 
//
// UnitTest.Register(<name>, <functor>, <module>);
// UnitTest.TestAll();
//
function phUnitTest() {
    if (this.arguments == 0 && typeof arguments[0] !== 'function') {
        throw new SyntaxError('Passed invalid function to be tested!');
    }

    this.functor = arguments[0];

    if (arguments.length > 1 && typeof arguments[1] !== 'undefined') {
        this.name = arguments[1].toString();
    }
    else {
    ...
    if (arguments.length > 2 && typeof arguments[2] !== 'undefined') {
        this.module = arguments[2].toString();
    }
}

phUnitTest.prototype.Register = function () {
    if (typeof this.module === 'string') {
        QUnit.module(this.module);
    }
    QUnit.test(this.name.toString(), this.functor);
}

function UnitTestController() {
    this.failedTests = [];
    this.listOfTests = [];
    this.requiredFileList = ['locales-en-US.xml', 'external/apa.csl', 'ama-separator-test.csl', 'external/ieee.csl', 'external/nature.csl'];
    this.fileDictionary = [];
...
UnitTestController.prototype.RegisterTest = function RegisterTest(name, functor, module) {
    var test = new phUnitTest(functor, name, module);
    this.listOfTests.push(test);
    return test;
}

UnitTestController.prototype.GetTestNames = function () {
    var index;
    var names = [];
    for (index = 0; index < this.listOfTests.length; ++index) {
        names.push(this.listOfTests[index].name);
    }
    return names;
}

UnitTestController.prototype.ResetQUnit = function () {
    this.failedTests = [];
    QUnit.reset();
    QUnit.init();
    QUnit.config.autostart = false;
...
UnitTestController.prototype.LoadFile = function(url, callback) {
    var isAsync = !!isWebEngine();
    var xhr = new XMLHttpRequest();
    if(isAsync) {
        xhr.onload = callback.bind({ xhr: xhr, unittest: this, url: url });
    }
    xhr.open('GET', url, isAsync);
    xhr.send(null);
    if(!isAsync) {
        callback.call({ xhr: xhr, unittest: this, url: url });
    }
}

UnitTestController.prototype.LoadCache = function() {
    var isAsync = !!isWebEngine();
    var counter = this.requiredFileList.length;

    var callback = function () {
        this.unittest.fileDictionary[this.url] = this.xhr.responseText;
        --counter;
        if (!counter && isAsync) {
            connectToMendeleyDesktop();
        }
    }

    this.requiredFileList.forEach(function (url) {
        this.LoadFile(url, callback);
    }.bind(this));
}

UnitTestController.prototype.TestNamed = function (name) {
    this.ResetQUnit();
    var found = false;
    name = name.toString();
    var index;
    for (index = 0; index < listOfTests.length; ++index) {
        var test = listOfTests[index];
        if (test.name == name) {
            found = true;
            test.Register();
        }
    ...
    if (found) {
        ...
        log("Test " + name + " not found!");
    }
}

UnitTestController.prototype.TestModule = function (module) {
    this.ResetQUnit();
    module = module.toString();
    var count = 0;
    this.listOfTests.forEach(function (test) {
...
UnitTestController.prototype.TestAll = function () {
    this.ResetQUnit();
    this.listOfTests.forEach(function (test) {
        test.Register();
    });
...
UnitTestController.prototype.Start = function () {
    QUnit.start(); 
}

UnitTestController.prototype.GetFile = function (url) {
    var logtext = "Looking for file: " + url;
    var text =  this.fileDictionary[url];
    if(!!text){
        logtext += " Found";
    } else {
        logtext += " Missing";
...
window.UnitTests = new UnitTestController();
//End of phUnitTest


// Test utility functions
var fetchTextSync = function (url) {

    if(!!isWebEngine()) {
        return UnitTests.GetFile(url);
    } 

    var output;

    var callback = function() {
        output = this.xhr.responseText;
    }

    UnitTests.LoadFile(url, callback);

    return output;
};

var newCiteprocReact = function () {
    var citeprocReact = new CiteprocReact({
        citeprocSys: {
            retrieveLocale: function (lang) {
                return fetchTextSync('locales-en-US.xml');
            }
        }
    });
    return citeprocReact;
}

var randomSeed = 0;
var pseudoRandomNumber = function (x) {
    if (!x) {
        x = randomSeed++;
    }
    return Math.abs(Math.sin(1000 * x) * 1000 % 1);
};

var pseudoRandomInteger = function (max, x) {
    if (!x) {
        x = randomSeed++;
    }
    return Math.round(max * pseudoRandomNumber(x));
};

var testCitationItem = function (number, extraProperties) {
    return _.extend({
        id: 'item-' + number,
        item: {
            id: 'item-' + number,
            ...
            author: [{
                family: ['Steve', 'Rob', 'Carles', 'George'][number % 4] + ' ' + ['Ridout', 'Knight', 'Pina', 'Vafiadis', 'Kartvelishvili'][number % 5]
            }],
            issued: {
                "date-parts": [[(1980 + Math.round(35 * pseudoRandomNumber(number)))]]
            }
        }
    }, extraProperties);
};

// Simplify citeproc bibliography entry HTML output
CSL.Output.Formats["html"]["@bibliography/entry"] = function (state, str) {
    return str;
};

UnitTests.RegisterTest("Normalise items", function (assert) {
    var testItems = [
        {
            title: 'hello',
            id: 'irrelevant'
    ...
    var clusterList = new ClusterList({ citationClusters: testItems });

    var normalisedTestItems = _.map(testItems, function (item) {
        return clusterList._prepareItem(item);
    });

    assert.ok(normalisedTestItems[0].id.length > 0);
    assert.deepEqual(normalisedTestItems[0], normalisedTestItems[1]);
    assert.notDeepEqual(normalisedTestItems[0], normalisedTestItems[2]);
}, "ClusterList");

UnitTests.RegisterTest("Prepare citation clusters", function (assert) {
    var testClusters = [
        {
            citationID: 'irrelevant 1',
            citationItems: [testCitationItem(1, { 'author-only': true })]
        },
        {
            citationID: 'irrelevant 2',
            citationItems: [testCitationItem(1), testCitationItem(2)]
        },
        {
            citationID: 'irrelevant 3',
            citationItems: [testCitationItem(1, { 'author-only': true })]
        }
    ];

    var clusterList = new ClusterList({ citationClusters: testClusters });

    assert.equal(clusterList.clusters[0].cluster.citationItems[0].id, "2afd54e6eed4841d084b50f9b2d4e6ba");
    assert.equal(clusterList.clusters[1].cluster.citationItems[0].id, clusterList.clusters[0].cluster.citationItems[0].id);
    assert.notEqual(clusterList.clusters[1].cluster.citationItems[1].id, clusterList.clusters[0].cluster.citationItems[0].id);

    assert.equal(
        clusterList.getItem(clusterList.clusters[0].cluster.citationItems[0].id).title,
        testCitationItem(1).item.title, 'getItem()');
    assert.equal(clusterList.clusters[0].cluster.citationItems[0]['author-only'], true);

    assert.notEqual(clusterList.clusters[0].clusterString, clusterList.clusters[1].clusterString, "non-equivalent clusters");
    assert.equal(clusterList.clusters[0].clusterString, clusterList.clusters[2].clusterString, "equivalent clusters");

    var citeprocClusters = clusterList.getCiteprocClusters();

    assert.equal(citeprocClusters[0].citationID, "CITATION-0", "citeproc citationID");
    assert.equal(citeprocClusters[1].citationID, "CITATION-1", "citeproc citationID");
    assert.equal(citeprocClusters[2].citationID, "CITATION-2", "citeproc citationID");

    assert.equal(citeprocClusters[0].citationItems[0].id, clusterList.clusters[0].cluster.citationItems[0].id,
        "citeproc itemID");
}, "ClusterList");

UnitTests.RegisterTest("Diff string lists", function (assert) {
    var list1 = [
            "one",
            "two",
            "three",
    ...
    var diffs = diffStringLists(list1, list2);

    assert.deepEqual(diffs, [
        { action: "equal", items: ["one", "two"] },
        { action: "remove", items: ["three"] },
        { action: "insert", items: ["new!"] },
        { action: "equal", items: ["four"] },
        { action: "remove", items: ["five"] }
    ]);
}, "ClusterList");

UnitTests.RegisterTest("Diff cluster change", function (assert) {
    var testClusters = [
            {
                citationID: 'irrelevant',
                citationItems: [testCitationItem(1, { 'author-only': true })]
            },
            {
                citationID: 'irrelevant',
            ...
            {
                citationID: 'irrelevant',
                citationItems: [testCitationItem(4)]
            },
            {
                citationID: 'irrelevant',
                citationItems: [testCitationItem(1, { 'author-only': true })]
        ...
        testClusters2 = deepCopy(testClusters);

    var clusterList1 = new ClusterList({ citationClusters: testClusters });
    ...
    var clusterList2 = new ClusterList({ citationClusters: testClusters2 });
    var diffs = ClusterList.prototype._diffClusterLists(clusterList1.clusters, clusterList2.clusters);

    assert.deepEqual(_.pluck(diffs, 'action'), ["equal", "remove", "insert", "equal", "remove"]);

    // render clusterList 1 (creates citeproc citationIDs)
    var citeprocClusters1 = clusterList1.getCiteprocClusters();

    assert.equal(citeprocClusters1[0].citationID, "CITATION-0", "citationID");
    assert.equal(citeprocClusters1[1].citationID, "CITATION-1", "citationID");
}, "ClusterList");

UnitTests.RegisterTest("Diff cluster change: removal of penultimate cluster", function (assert) {
    var testClusters = [
            {
                citationID: 'irrelevant',
                citationItems: [testCitationItem(1, { 'author-only': true })]
    ...
    testClusters2.splice(2, 1);

    var clusterList1 = new ClusterList({ citationClusters: testClusters });
    clusterList1.getCiteprocClusters();

    assert.equal(clusterList1.clusters.length, 4);
    assert.equal(clusterList1.clusters[3].cluster.citationID, "CITATION-3");

    var clusterList2 = new ClusterList({ citationClusters: testClusters2 });

    var changes = clusterList2.minimalCiteprocChanges(clusterList1);

    assert.equal(changes.length, 1, "one change");
    var change = changes.pop();
    assert.equal(change.citation.citationID, "CITATION-3");
    assert.deepEqual(_.pluck(change.citationsPre, 0), ["CITATION-0", "CITATION-1"]);
    assert.deepEqual(_.pluck(change.citationsPost, 0), []);
}, "ClusterList");

UnitTests.RegisterTest("Diff cluster change: removal of last cluster", function (assert) {
    var testClusters = [
            {
                citationID: 'irrelevant',
                citationItems: [testCitationItem(1, { 'author-only': true })]
    ...
    assert.equal(changes.length, 1, "one change");
    var change = changes.pop();
    assert.equal(change.citation.citationID, "CITATION-2");
    assert.deepEqual(_.pluck(change.citationsPre, 0), ["CITATION-0", "CITATION-1"]);
    assert.deepEqual(_.pluck(change.citationsPost, 0), []);
}, "ClusterList");

UnitTests.RegisterTest("Diff cluster change: append cluster", function (assert) {
    var testClusters = [
            {
                citationID: 'irrelevant',
                citationItems: [testCitationItem(1, { 'author-only': true })]
    ...
    testClusters2.push({ citationItems: [testCitationItem(3)] });
...
UnitTests.RegisterTest("Diff cluster change: prepend cluster", function (assert) {
    var testClusters = [
            {
                citationID: 'irrelevant',
                citationItems: [testCitationItem(1, { 'author-only': true })]
    ...
    testClusters2.splice(0, 0, { citationItems: [testCitationItem(3)] });
    ...
    var change = changes.pop();
    assert.equal(change.citation.citationID, "CITATION-2");
    assert.deepEqual(_.pluck(change.citationsPre, 0), []);
    assert.deepEqual(_.pluck(change.citationsPost, 0), ["CITATION-0", "CITATION-1"]);
}, "ClusterList");

var doRenderingTest = function (assert, alwaysReset) {
    var style = fetchTextSync('external/apa.csl');
    var citeprocReact = newCiteprocReact();
    ...
    var testClusters = [
            ...
            {
                citationItems: [testCitationItem(3, { 'author-only': true })]
            }
    ];

    var result = citeprocReact.render({
        style: style,
        citationClusters: testClusters,
        reset: alwaysReset,
        bibliographyOutputFormat: "html"
    });

    assert.equal(result.formattedCitations.length, 3, 'formatted citations length');
    assert.equal(result.formattedCitations[0], "Rob Knight");
    assert.equal(result.formattedCitations[1], "(Carles Pina, 1981; Rob Knight, 2011)");
    assert.equal(result.formattedCitations[2], "George Vafiadis");

    assert.equal(result.formattedBibliography.length, 3, 'formatted bibliography length');
    assert.equal(result.formattedBibliography[0], "Carles Pina. (1981). Article 2.");
    assert.equal(result.formattedBibliography[1], "George Vafiadis. (1987). Article 3.");
    assert.equal(result.formattedBibliography[2], "Rob Knight. (2011). Article 1.");

    // Alter by appending a citation cluster
    testClusters.push({ citationItems: [testCitationItem(4), testCitationItem(2)] });

    // Re-render
    result = citeprocReact.render({
        style: style,
        citationClusters: testClusters,
        reset: alwaysReset,
        bibliographyOutputFormat: "html"
    ...
    assert.equal(result.formattedCitations.length, 4, 'formatted citations length');
    assert.equal(result.formattedCitations[0], "Rob Knight");
    assert.equal(result.formattedCitations[1], "(Carles Pina, 1981; Rob Knight, 2011)");
    assert.equal(result.formattedCitations[2], "George Vafiadis");
    assert.equal(result.formattedCitations[3], "(Carles Pina, 1981; Steve Kartvelishvili, 1998)");

    assert.equal(result.formattedBibliography.length, 4, 'formatted bibliography length');
    assert.equal(result.formattedBibliography[0], "Carles Pina. (1981). Article 2.");
    assert.equal(result.formattedBibliography[1], "George Vafiadis. (1987). Article 3.");
    assert.equal(result.formattedBibliography[2], "Rob Knight. (2011). Article 1.");
    assert.equal(result.formattedBibliography[3], "Steve Kartvelishvili. (1998). Article 4.");

    // Alter by editing a citation cluster
    testClusters[1] = { citationItems: [testCitationItem(5), testCitationItem(2)] };
    ...
    assert.equal(result.formattedCitations.length, 4, 'edit cluster: formatted citations length');
    assert.equal(result.formattedCitations[0], "Rob Knight");
    assert.equal(result.formattedCitations[1], "(Carles Pina, 1981; Rob Ridout, 2014)");
    assert.equal(result.formattedCitations[2], "George Vafiadis");
    assert.equal(result.formattedCitations[3], "(Carles Pina, 1981; Steve Kartvelishvili, 1998)");

    assert.equal(result.formattedBibliography.length, 5, 'edit cluster: formatted bibliography length');
    assert.equal(result.formattedBibliography[0], "Carles Pina. (1981). Article 2.");
    assert.equal(result.formattedBibliography[1], "George Vafiadis. (1987). Article 3.");
    assert.equal(result.formattedBibliography[2], "Rob Knight. (2011). Article 1.");
    assert.equal(result.formattedBibliography[3], "Rob Ridout. (2014). Article 5.");
    assert.equal(result.formattedBibliography[4], "Steve Kartvelishvili. (1998). Article 4.");

    // Completely new list
    result = citeprocReact.render({
        style: style,
        citationClusters: [
            { citationItems: [testCitationItem(6)] },
            { citationItems: [testCitationItem(7)] },
            { citationItems: [testCitationItem(8), testCitationItem(9)] },
        ],
        reset: alwaysReset,
        bibliographyOutputFormat: "html"
    });

    assert.equal(result.formattedCitations.length, 3, 'change all: formatted citations length');
    assert.equal(result.formattedCitations[0], "(Carles Knight, 2005)");
    assert.equal(result.formattedCitations[1], "(George Pina, 2011)");
    assert.equal(result.formattedCitations[2], "(Rob Kartvelishvili, 1996; Steve Vafiadis, 2010)");

    assert.equal(result.formattedBibliography.length, 4, 'change all: formatted bibliography length');
    assert.equal(result.formattedBibliography[0], "Carles Knight. (2005). Article 6.");
    assert.equal(result.formattedBibliography[1], "George Pina. (2011). Article 7.");
    assert.equal(result.formattedBibliography[2], "Rob Kartvelishvili. (1996). Article 9.");
    assert.equal(result.formattedBibliography[3], "Steve Vafiadis. (2010). Article 8.");
    ...
    assert.equal(result.formattedCitations.length, 0, 'remove all citation clusters');
    assert.equal(result.formattedBibliography.length, 0, 'remove all citation clusters');
};

UnitTests.RegisterTest("Render small test documents: always reset", function (assert) {
    doRenderingTest(assert, true);
}, "CiteprocReact");

UnitTests.RegisterTest("Render small test documents: incremental updates", function (assert) {
    doRenderingTest(assert, false);
}, "CiteprocReact");

UnitTests.RegisterTest("incremental render - add 2 identical new items", function (assert) {
    var style = fetchTextSync('external/apa.csl');
    var citeprocReact = newCiteprocReact();
    ...
    var testItems = {};
    testItems[testCitationItem(1).id] = testCitationItem(1).item;
    ...
    assert.equal(result.formattedCitations.length, 1);
    assert.equal(result.formattedCitations[0], "Rob Knight");

    assert.equal(result.formattedBibliography.length, 1);
    assert.equal(result.formattedBibliography[0], "Rob Knight. (2011). Article 1.");

    testClusters.push({ citationItems: [{ id: 'new-item-id' }] });
    testClusters.push({ citationItems: [{ id: 'new-item-id' }] });

    testItems['new-item-id'] = { title: 'new title', author: [{ family: 'new author' }] };
    ...
    assert.equal(result.formattedCitations.length, 3);
    assert.equal(result.formattedCitations[0], "Rob Knight");
    assert.equal(result.formattedCitations[1], "(new author, n.d.)");
    assert.equal(result.formattedCitations[2], "(new author, n.d.)");

    assert.equal(result.formattedBibliography.length, 2);
    assert.equal(result.formattedBibliography[0], "new author. (n.d.). new title.");
    assert.equal(result.formattedBibliography[1], "Rob Knight. (2011). Article 1.");
}, "CiteprocReact");

UnitTests.RegisterTest("incremental render - different citation IDs", function (assert) {
    var style = fetchTextSync('external/apa.csl');
    var citeprocReact = newCiteprocReact();

    var testClusters = [
            {
                citationID: 'test-id-1',
                citationItems: [testCitationItem(1, { 'author-only': true })]
            }
    ];
    var testItems = {};
    ...
    // same as first but with different id
    testClusters[0].citationID = "changed-id";

    var result2 = citeprocReact.render({
        style: style,
        citationClusters: testClusters,
        items: testItems,
        bibliographyOutputFormat: "html"
    ...
    assert.deepEqual(_.omit(result, 'info'), _.omit(result2, 'info'));
}, "CiteprocReact");

UnitTests.RegisterTest("render empty list", function (assert) {
    var style = fetchTextSync('external/apa.csl');
    var citeprocReact = newCiteprocReact();
    ...
    assert.equal(result.formattedCitations.length, 0);
    assert.equal(result.formattedBibliography.length, 0);
    ...
    assert.equal(result2.formattedCitations.length, 0);
    assert.equal(result2.formattedBibliography.length, 0);
}, "CiteprocReact");

UnitTests.RegisterTest("render unicode separator", function (assert) {
    var style = fetchTextSync('ama-separator-test.csl');
    var citeprocReact = newCiteprocReact();
    ...
    var char1 = String.fromCharCode(237),
        char2 = String.fromCharCode(224);

    assert.ok(result.formattedBibliography.join("").indexOf(char1) !== -1, "contains " + char1);
    assert.ok(result.formattedBibliography.join("").indexOf(char2) !== -1, "contains " + char2);
}, "CiteprocReact");

UnitTests.RegisterTest("change styles", function (assert) {
    var citeprocReact = newCiteprocReact();
    ...
    _.each(['apa', 'ieee', 'nature'], function (styleName) {
        var startTime = new Date().getTime();
        var result = citeprocReact.render({
            style: fetchTextSync('external/' + styleName + '.csl'),
            citationClusters: testClusters,
            bibliographyOutputFormat: "html"
        });
        var fullAPARenderTime = new Date().getTime() - startTime;
        assert.equal(result.info.type, 'full', 'full render');
        ...
        var incrementalAPARenderTime = new Date().getTime() - startTime;
        assert.equal(result.info.type, 'incremental', 'incremental render');
        assert.ok(incrementalAPARenderTime < fullAPARenderTime, 'incremental faster than full render');
    });
}, "CiteprocReact");

UnitTests.RegisterTest("extra (e.g. mendeley) citation cluster fields", function (assert) {
    var style = fetchTextSync('external/apa.csl');
    var citeprocReact = newCiteprocReact();
    ...
    var testClusters = [
            ...
            },
            {
                citationID: 'irrelevant',
                citationItems: [testCitationItem(1), testCitationItem(2)],
                mendeley: 'ignore this',
                arbiraryOtherField: "pay no heed"
            }
    ];
    ...
    assert.equal(result.info.type, 'full');

    testClusters[0].mendeley = 'new';
    testClusters[1].mendeley = 'new';
    ...
    assert.equal(result.info.type, 'incremental');
    assert.equal(result.info.numChanges, 0);
}, "CiteprocReact");

UnitTests.RegisterTest("Render bibliography only", function (assert) {
    var style = fetchTextSync('external/apa.csl');
    var citeprocReact = newCiteprocReact();

    var originalTestItems = _.map(_.range(5), testCitationItem);
    var testItems = {};

    var index = 1;
    // set item keys correctly for bibliography test
    _.each(originalTestItems, function (item) {
        var id = "ITEM-" + index;
        testItems[id] = item.item;
        testItems[id].id = id;
        index++;
    });
    var testClusters = _.map(testItems, function (item) {
        return {
            citationItems: [item]
        }
    });

    var fullRenderResult = citeprocReact.render({
        style: style,
        citationClusters: testClusters,
        items: deepCopy(testItems),
        bibliographyOutputFormat: "html"
    });
    var bibliographyOnlyResult = citeprocReact.render({
        style: style,
        citationClusters: [],
        items: deepCopy(testItems),
        reset: true,
    ...
    assert.deepEqual(bibliographyOnlyResult.formattedBibliography, fullRenderResult.formattedBibliography);
}, "CiteprocReact");

QUnit.testDone(function (data) {
    if (data.failed > 0) {
        UnitTests.failedTests.push(data.name);
    }
});

QUnit.done(function (data) {
    if (!isWebEngine() && typeof (MendeleyDesktop_) !== "undefined") {
        // if isWebEngine() then it's done when connecting the channel.
        MendeleyDesktop = MendeleyDesktop_;
    }

    if (typeof (MendeleyDesktop) !== "undefined" && !!MendeleyDesktop) {
        MendeleyDesktop.unitTestsFinished(data.passed, data.total, UnitTests.failedTests.join('\n'));
    }
});

// ---- slow tests start here ----

/*
var largeDocumentRenderingTest = function (styleName, assert, alwaysReset) {
    var style = fetchTextSync('external/' + styleName + '.csl');
    var citeprocReact = newCiteprocReact();
    
    var testClusters = JSON.parse(fetchTextSync('testCitations.json'));
    var testItems = JSON.parse(fetchTextSync('testItems.json'));
    
    // set item keys correctly for bibliography test
    _.each(testItems, function (item, key) {
        item.id = key;
    });
    
    var result1 = citeprocReact.render({
        style: style,
        citationClusters: testClusters,
        items: testItems,
        reset: alwaysReset
    ...
    assert.deepEqual(_.omit(result1, 'info'), _.omit(result2, 'info'));
    
    // Alter by appending and prepending a citation cluster
    testClusters.splice(0, 0, {citationItems: [testCitationItem(1), testCitationItem(2)]});
    
    console.time('large document prepend. alwaysReset=' + alwaysReset + ', style=' + styleName);
    var result3 = citeprocReact.render({
        style: style,
        citationClusters: testClusters,
        items: testItems,
        reset: alwaysReset
    });
    console.timeEnd('large document prepend. alwaysReset=' + alwaysReset + ', style=' + styleName);
    
    testClusters.push({citationItems: [testCitationItem(3), testCitationItem(4)]});
    
    console.time('large document append. alwaysReset=' + alwaysReset + ', style=' + styleName);
    var result4 = citeprocReact.render({
        style: style,
        citationClusters: testClusters,
        items: testItems,
        reset: alwaysReset
    });
    console.timeEnd('large document append. alwaysReset=' + alwaysReset + ', style=' + styleName);
    
    assert.equal(result4.formattedCitations.length, result2.formattedCitations.length + 2);
    assert.equal(result4.formattedBibliography.length, result2.formattedBibliography.length + 4);
};
    
QUnit.test("Render large document: always reset", function (assert) {
    log('---- render large - full render');
    largeDocumentRenderingTest('apa', assert, true);
});
    
QUnit.test("Render large document: incremental updates", function (assert) {
    log('---- render large - incremental render');
    largeDocumentRenderingTest('apa', assert, false);
});
    
// A brute force search for problems with incremental rendering...
QUnit.test("incremental render - lots of random documents", function (assert) {
    var style = fetchTextSync('external/apa.csl');
    var citeprocReact = newCiteprocReact();
    
    // lots of random clusterLists: compare
    var randomSeed = 0;
    
    var NUM_DOCUMENTS = 20, // increase significantly for better coverage
        MAX_CLUSTERS_PER_DOCUMENT = 100,
        MAX_ITEMS_PER_CLUSTER = 6;
        
    var randomClusterList = function () {
        return _.map(_.range(pseudoRandomInteger(MAX_CLUSTERS_PER_DOCUMENT)), function (cluster) {
            var citationItems = _.map(_.range(1 + pseudoRandomInteger(MAX_ITEMS_PER_CLUSTER - 1)), function (item) {
                return testCitationItem(pseudoRandomInteger(100));
            });
    
            return {
                citationItems: citationItems,
                properties: {},
                citationID: "this is ignored"
            };
        });
    };
    
    var bunchOfRandomClusterLists = _.map(_.range(NUM_DOCUMENTS), function () {
        return randomClusterList();
    });
    
    var originalResults = [];
    
    // Run in reset mode - full re-render each time
    console.time('random documents - reset mode');
    _.each(bunchOfRandomClusterLists, function (clusters, index) {
        var result = citeprocReact.render({
            style: style,
        ...
        originalResults.push(_.omit(result, 'info'));
    });
    console.timeEnd('random documents - reset mode');
    
    // Re-run in incremental mode and compare to original
    console.time('random documents - inc mode');
    _.each(bunchOfRandomClusterLists, function (clusters, index) {
        var result = citeprocReact.render({
            style: style,
            citationClusters: clusters
        ...
        assert.deepEqual(_.omit(result, 'info'), originalResults[index]);
    });
    console.timeEnd('random documents - inc mode');
});
    
var benchmark = function (styleName, assert, alwaysReset, time) {
    var style = fetchTextSync('external/' + styleName + '.csl');
    var citeprocReact = newCiteprocReact();
    ...
    var renderType = alwaysReset ? "full" : "incremental";
    
    time("prepend - " + styleName + " - " + renderType, function () {
        var result3 = citeprocReact.render({
            style: style,
            citationClusters: testClusters,
            items: testItems,
    ...
    var result4;
    time("append - " + styleName + " - " + renderType, function () {
        result4 = citeprocReact.render({
            style: style,
            citationClusters: testClusters,
            items: testItems,
            reset: alwaysReset
    ...
    assert.equal(result4.formattedCitations.length, result1.formattedCitations.length + 2);
    assert.equal(result4.formattedBibliography.length, result1.formattedBibliography.length + 4);
};
    
QUnit.test("Benchmarking test", function (assert) {
    var results = {};
    
    var time = function (description, callback) {
        var startTime = new Date().getTime();
        callback();
        var totalTime = new Date().getTime() - startTime;
    
        results[description] = results[description] || [];
        results[description].push(totalTime);
    
        log(description + ": " + totalTime + "ms");
    };
    
    benchmark('apa', assert, true, time);
    benchmark('apa', assert, false, time);
    benchmark('apa', assert, true, time);
    benchmark('apa', assert, false, time);
    benchmark('apa', assert, true, time);
    benchmark('apa', assert, false, time);
    
    benchmark('ieee', assert, true, time);
    benchmark('ieee', assert, false, time);
    benchmark('ieee', assert, true, time);
    benchmark('ieee', assert, false, time);
    benchmark('ieee', assert, true, time);
    benchmark('ieee', assert, false, time);
    ...
    var averagedResults = _.map(results, function (times, description) {
        var average = 0;
        _.each(times, function (value) {
            average += value;
        });
    ...
    log('averaged results: ', averagedResults);
});
*/
